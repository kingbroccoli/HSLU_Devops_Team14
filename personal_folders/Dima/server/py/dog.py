...
class Dog(Game):
    ...
    def get_list_action(self) -> List[Action]:
        ...
        partner_idx = (idx + 2) % 4
        for c in player.list_card:
            if c.rank.isdigit():  # Only number cards can help partner
                steps = int(c.rank)
                for m in self.state.list_player[partner_idx].list_marble:
                    if not self.is_in_kennel(m.pos, partner_idx) and not self.is_in_finish(m.pos, partner_idx):
                        pos_to = self.calculate_move(partner_idx, m.pos, steps)
                        if self.is_move_valid(partner_idx, m.pos, pos_to, steps, c):
                            actions.append(Action(card=c, pos_from=m.pos, pos_to=pos_to, card_swap=None))
    ...
    def get_marble_at_pos(self, pos: int):
        for p_idx, p in enumerate(self.state.list_player):
            for mm in p.list_marble:
                if mm.pos == pos:
                    return mm  # Return marble instead of position
        return None
    ...
    def calculate_move(self, idx_player: int, pos: int, steps: int):
        if pos < 64:
            raw_pos = (pos + steps) % 64
            # Check if marble should enter finish area
            if idx_player * 16 <= pos < (idx_player * 16 + 16) and raw_pos > idx_player * 16:
                finish_start = 64 + idx_player * 8 + 4
                finish_pos = finish_start + (raw_pos - idx_player * 16 - 1)
                if finish_pos < finish_start + 4:
                    return finish_pos
            return raw_pos
        return pos + steps 
