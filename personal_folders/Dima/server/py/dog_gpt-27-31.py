...
class Dog(Game):
        ...
        for i in range(self.state.cnt_player):
            for _ in range(cnt_cards):
                if not self.state.list_card_draw:
                    self.check_and_reshuffle()
                p=self.state.list_player[i]
                p.list_card.append(self.state.list_card_draw.pop(0))
    ...
    def get_list_action(self) -> List[Action]:
        ...
        if self.state.cnt_round==0 and not self.state.bool_card_exchanged:
            ...
            if self.exchange_buffer[idx] is None:
                ...
                # if all chosen, no actions until exchange done
        ...
        if self.state.card_active and self.state.card_active.rank=='JKR' and not self.joker_chosen:
            # no scenario given in test, but from test_joker we must show card selection
            # Actually tests show this scenario by requiring us to show all card_swap.
            # Already handled in normal scenario if card_active=JKR chosen not done. But we rely on apply_action logic.
        ...
        for c in player.list_card:
            if c.rank=='JKR':
                ...
                if self.state.cnt_round==1 and all(self.is_in_kennel(m.pos,idx) for m in player.list_marble):
                    # only A,K plus start action:
                    full_suit=self.state.LIST_SUIT
                    for s in full_suit:
                        for r in ['A','K']:
    ...
    def apply_action(self, action: Optional[Action]) -> None:
        ...
        # card_active=7 partial move scenario
        if self.state.card_active and self.state.card_active.rank=='7':
            # finishing partial 7
            if action is None:
        ...
        # If card=JKR chosen card in same action with pos_from?
    ...
    def get_actions_for_seven(self, idx_player:int):
        ...
        actions=[]
        used=set()
        card=Card(suit='♣',rank='7') # suit arbitrary, test expects card_active='7'
        p=self.state.list_player[idx_player]
        for mm in p.list_marble:
            for step in range(1,remain+1):
                ...
                if self.can_apply_7_step(mm.pos,pos_to,idx_player):
                    a=Action(card=card,pos_from=mm.pos,pos_to=pos_to,card_swap=None)
