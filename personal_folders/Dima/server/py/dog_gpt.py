...
class Dog(Game):
    ...
    def reset(self):
        ...
        # Deal cards for round 1: 6 cards each
    ...
    def get_list_action(self) -> List[Action]:
        ...
        # If not exchanged yet
        if not self.state.bool_card_exchanged:
            # if this player not chosen a card yet:
        ...
        # If card_active=JKR but no chosen card yet (should not happen since we choose card_swap in apply_action)
        if self.state.card_active and self.state.card_active.rank=='JKR' and not self.joker_chosen:
            # no scenario given
            return []

        # Normal turn: no card_active
        actions=[]
        used=set()
        for c in player.list_card:
            if c.rank=='JKR':
                ...
                # JKR card_swap:
                # Beginning scenario show A,K as test requires:
                # Also show full set if round>1 or marbles out:
                LIST_SUIT=self.state.LIST_SUIT
                if self.state.cnt_round==1 and all(self.is_in_kennel(m.pos,idx) for m in player.list_marble):
                    # only show A,K replacements plus start action:
                    for s in LIST_SUIT:
                        for r in ['A','K']:
                    ...
                    # show full set of ranks (2..A,K,J,Q,10 etc.)
    ...
    def apply_action(self, action: Optional[Action]) -> None:
        ...
        # card_active=JKR and no chosen card yet:
        ...
        if self.state.card_active and self.state.card_active.rank=='7':
            ...
            if action.card.rank=='7':
                ...
                if not success:
                    self.restore_backup_state()
                    self.end_turn()
                # if steps done end_turn handled in apply_seven_step
            else:
                return
            return
        ...
        if self.state.card_active and self.joker_chosen and self.state.card_active.rank!='7':
            # normal move with chosen card
            if action is None:
                # fold?
                self.end_turn()
                return
            if self.apply_normal_move(action):
                # remove no card from hand because we replaced JKR?
                # The JKR was removed earlier, now we do not have the chosen card in hand actually.
                # According to test 028, remove one JKR from hand was done.
                # just discard chosen card
                self.state.card_active=None
                self.joker_chosen=False
                self.end_turn()
            return
        ...
        # If card=7 and no partial started yet
        if action.card.rank=='7':
            if not self.has_card_in_hand(idx,action.card):
                return
            self.save_backup_state()
        ...
        # If card=JKR and action chosen start from kennel
        ...
        # if card=JKR and action.card_swap chosen in same action? This is handled above
    ...
    def is_in_finish(self, pos:int, idx_player:int):
        ...
        return pos>=finish_start and pos<finish_end
            ...
            if can_start:
                ...
                for km_pos in kennel_positions:
                    if self.own_marble_at_pos(idx_player, km_pos):
                        actions.append(Action(card=card,pos_from=km_pos,pos_to=start_field,card_swap=None))
        return actions
    ...
    def get_normal_moves(self, idx_player:int, card:Card):
        ...
        for mm in p.list_marble:
            if True: # all marbles considered, validity check inside
                for st in steps_options:
                    ...
                    if self.is_move_valid(idx_player,mm.pos,pos_to,st,card):
                        actions.append(Action(card=card,pos_from=mm.pos,pos_to=pos_to,card_swap=None))
        return actions

    def get_j_actions(self, idx_player:int, card:Card):
        actions=[]
        # find swappable marbles
        marbles_info=[]
    ...
    def get_actions_for_seven(self, idx_player:int):
        ...
        actions=[]
        used=set()
        card=Card(suit='♣',rank='7') # suit arbitrary
        p=self.state.list_player[idx_player]
        for mm in p.list_marble:
    ...
    def get_actions_for_card(self, card:Card, idx_player:int):
        ...
        used=set()
        if card.rank in ['A','K']:
            start_actions=self.get_start_actions(idx_player,card)
            for a in start_actions:
    ...
    def is_in_kennel(self, pos:int, idx_player:int):
        kennel_start=64+idx_player*8
        return pos>=kennel_start and pos<kennel_start+4
    ...
    def has_marble_in_kennel(self, idx_player:int):
        ...
        p=self.state.list_player[idx_player]
        for mm in p.list_marble:
            if mm.pos>=kennel_start and mm.pos<kennel_start+4:
                return True
        return False
    ...
    def calculate_move(self, idx_player:int, pos:int, steps:int):
        ...
        if pos<64:
            ...
            finish_start=64+idx_player*8+4
            finish_end=finish_start+4
            if raw_pos>=finish_start and raw_pos<finish_end:
                final=raw_pos
            else:
                final=(pos+steps)%64
        else:
    ...
    def is_move_valid(self, idx_player:int, pos_from:int, pos_to:int, steps:int, card:Card):
        ...
        if self.is_in_finish(pos_to,idx_player) and steps<0:
            return False
        path=self.get_path_positions(min(pos_from,pos_to),max(pos_from,pos_to))
        # Check blocking save marble on start:
        # Actually we must check all positions we pass over.
        # If forward steps>0:
        if steps>0:
