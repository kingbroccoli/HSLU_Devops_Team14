...
class HangmanBenchmark(Benchmark):
    ...
    def test_action_list(self) -> None:
        ...
        assert {action.letter for action in actions} == set(string.ascii_lowercase), hint
        ...
        assert {action.letter for action in actions1} == set(string.ascii_lowercase[3:]), hint

        # testcase 2
        test_state2 = HangmanGameState(word_to_guess="", guesses=[*string.ascii_lowercase], phase=GamePhase.RUNNING)
